...
import { DEFAULT_IMAGE_MAX_BYTES, DEFAULT_IMAGE_PIXEL_BUDGET, DEFAULT_REASONING_EFFORTS, DEFAULT_STREAM_IDLE_TIMEOUT_MS, OFF_EFFORT, } from './config.js';
import { readCredential } from './credential.js';
import { classifyFailure, classifyTransportError } from './failure.js';
import { buildRequestBody, collectImageRefs } from './serialize.js';
...
export class AppCredentialsAdapter extends LlmAdapter {
    ...
    async serializeHelpers(profile, options) {
        ...
        if (store === undefined) {
            throw new LlmError(`${PKG}: model "${options.model}" accepts image input, but the durable attachment service is not available`, 'UNSUPPORTED_CONTENT');
        }
        const policy = {
