 ...
 */
export async function readCredential(spec) {
    let document;
    try {
        document = JSON.parse(await readFile(spec.file, 'utf8'));
    }
    catch (error) {
        throw new LlmError(`${PKG}: cannot read the credential file "${spec.file}" (${describeError(error)}). ` +
            `Sign in to the source application again, or point the route's credential.file at the right file.`, 'MISSING_CREDENTIAL', { cause: error });
    }
    const raw = pick(document, spec.token);
    if (typeof raw !== 'string' || raw.length === 0) {
        throw new LlmError(`${PKG}: the credential file "${spec.file}" carries no token at "${spec.token}". ` +
            `Check credential.token against the file's actual shape, or sign in again.`, 'MISSING_CREDENTIAL');
    }
    ...
    if (typeof expiresAt === 'number' && Number.isFinite(expiresAt) && expiresAt > 0 && Date.now() > expiresAt) {
        throw new LlmError(`${PKG}: the credential in "${spec.file}" expired at ${new Date(expiresAt).toISOString()}; ` +
            `sign in to the source application again to refresh it.`, 'AUTH');
    }
    return {
