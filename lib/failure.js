 ...
 */
import { LlmError, ProviderRequestId, isContextWindowExceededError, isQuotaExceededError, } from '@deepseek-ai/dsh-llm';
...
export async function classifyFailure(response) {
    ...
    if (illegalChannel !== undefined)
        return new LlmError(illegalChannel, 'INVALID_REQUEST', options);
    if (status === 401 || status === 403)
        return new LlmError(raw, 'AUTH', options);
    if (status === 429)
        return new LlmError(raw, 'RATE_LIMIT', options);
    if (isContextWindowExceededError(detail))
        return new LlmError(raw, 'CONTEXT_WINDOW_EXCEEDED', options);
    if (isQuotaExceededError(detail))
        return new LlmError(raw, 'QUOTA', options);
    if (status === 400)
        return new LlmError(raw, 'INVALID_REQUEST', options);
    if (status >= 500)
        return new LlmError(raw, 'SERVER', options);
 ...
 */
export function classifyTransportError(error, endpoint) {
    const summary = error instanceof Error ? error.message : String(error);
    return new LlmError(`${PKG}: cannot reach ${endpoint} (${summary}). Check the route's baseURL and, if this machine needs one, the desktop proxy setting.`, 'TRANSPORT', { cause: error });
}
...
function illegalChannelHint(raw) {
    if (!ILLEGAL_CHANNEL_PATTERN.test(raw) && !raw.includes('11128'))
        return undefined;
    return (`${PKG}: the upstream refused this client as an unapproved channel. Two known causes: ` +
...
function optionalRequestId(header) {
