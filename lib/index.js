...
export function apply(ctx, config) {
    ...
    let current = () => ({ providers: mergeProviders(detectProviders(), config.providers) });
    ...
    ctx.inject(['settings'], (settingsCtx) => {
        try {
            ...
            ctx.logger.error(`llm-app-credentials: settings namespace "${NS}" could not be registered, so the settings page will show it as unavailable. ` +
 ...
 */
function mergeProviders(detected, configured) {
...
function isImageStore(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    return typeof value.readImageRequest === 'function';
}
/**
...
function logBuildInfo(ctx) {
    ...
    ctx.logger.info(`llm-app-credentials: loaded build ${info.commitShort ?? 'unknown'}${dirty} ` +
...
function readBuildInfo() {
    try {
        ...
        const info = require('../lib/build-info.json');
