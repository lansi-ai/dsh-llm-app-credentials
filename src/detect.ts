...
/** One application the plugin knows how to look for. */
interface KnownSource {
  /** Default route name; a user config may reuse or rename it. */
  route: string
  /** Default display name for model selectors. */
  displayName: string
  /** Upstream root for this deployment. */
  baseURL: string
  /** File name inside the source application's auth directory. */
  authFileName: string
  /** Advertised model catalog (advisory; the upstream accepts unlisted ids too). */
  models: ModelSpec[]
}
...
const CREDENTIAL_PATHS = { token: 'auth.accessToken', uid: 'account.uid', expiresAt: 'auth.expiresAt' } as const

/** The directory those applications keep their credential files in, or undefined when undetectable. */
export function authDirectory(): string | undefined {
