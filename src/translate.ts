...
/** One block being assembled, keyed by its first-seen stream index. */
interface OpenBlock {
  index: number
  kind: 'text' | 'reasoning' | 'tool-call'
  /** Accumulated text (for `tool-call`, the raw JSON argument string). */
  text: string
  /** Tool-call identity, established by the call's first delta. */
  callId?: string
  name?: string
}
...
export async function* translate(chunks: AsyncIterable<WireChunk>): AsyncGenerator<StreamChunk> {
  ...
  for await (const chunk of chunks) {
    if (chunk.error !== undefined) throw inBandError(chunk)
...
function inBandError(chunk: WireChunk): LlmError {
